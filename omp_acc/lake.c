...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
...
#ifdef _OPENMP
	//parallelizing memcopies using openmp
	#pragma omp parallel for num_threads(nthreads) private(idx, j)
	for(i=0; i<n; i++)
		for(j=0; j<n; j++)
		{
			idx = i*n + j;
  ...
  dt = h / 2.;
	...
	//open acc copying data from cpu to gpu
	#pragma acc data copy(uc[:n*n]) copyin(un[:n*n],uo[:n*n],pebbles[:n*n])
  ...
  while(1)
  {
     ...
     * the wave equation in 2D */
		//for parallelizing both the loops
		//#pragma omp parallel for collapse(2) num_threads(nthreads) private(idx)

		//for parallelizing only outer loop
		#pragma omp parallel for num_threads(nthreads) private(idx, j)

		//dividing work and creating threads for each in gpu  
		#pragma acc kernels loop present(uc[:n*n],un[:n*n],uo[:n*n],pebbles[:n*n])
    for( i = 0; i < n; i++)
    {
			//for parallizing only inner loop
			//#pragma omp parallel for num_threads(nthreads) private(idx)
      for( j = 0; j < n; j++)
      {
        idx = j + i * n;
	...
	//pointing un to memory storing values of un before while loop broke
	un = uc;
